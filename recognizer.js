...
// 🔥 precompute tensors (big performance win)
let embeddingTensors = {};
...
async function loadEmbeddings() {
    ...
    // 🔥 convert once → avoid recreating tensors every frame
    for (const [name, vector] of Object.entries(embeddings)) {
        embeddingTensors[name] = tf.tensor1d(vector);
    }
}
...
function findBestMatch(embedding, threshold = 0.6) {
    ...
    for (const [name, tensor] of Object.entries(embeddingTensors)) {

        const score = cosineSimilarityTensor(inputTensor, tensor);

        // 🚫 disable spam logging (re-enable only if debugging)
